...
function obtenerUbicacion() {
  ...
  navigator.geolocation.getCurrentPosition(
    ...
    () => {
      // Sin permiso — distancia no disponible
      userLat = null;
      userLng = null;
    }
  );
...
function abrirFerias() {
  renderListado('ferias');
  showScreen('screen-listado');
}

function abrirDormir() {
  renderListado('dormir');
  showScreen('screen-listado');
}
...
let listadoTipoActual = '';

function renderListado(tipo) {
  listadoTipoActual = tipo;
  const items = DATA[tipo];
  const t = UI[currentLang];

  const titulos = {
    ferias: { es: '🛍️ Ferias y Paseos', en: '🛍️ Fairs & Markets',   pt: '🛍️ Feiras e Passeios' },
    dormir: { es: '🏨 Dónde Dormir',    en: '🏨 Where to Sleep',     pt: '🏨 Onde Dormir' },
    comer:  { es: '🍽️ Dónde Comer',     en: '🍽️ Where to Eat',      pt: '🍽️ Onde Comer' }
  };

  const esFeria = tipo === 'ferias';
  const titulo  = titulos[tipo][currentLang];
  ...
  const itemsOrdenados = [...items].map((item, idx) => ({
    ...
    const da = parseFloat(a._dist);
    const db = parseFloat(b._dist);
    return da - db;
  });

  itemsOrdenados.forEach(item => {
    const distStr = item._dist
      ? `📍 ${item._dist} ${t.distancia}`
      : esFeria
        ? `📅 ${item.dias[currentLang]} · ⏰ ${item.horario}`
        : `📍 Rosario`;
    ...
    const iconoHTML = `<div class="listado-logo">${item.icono}</div>`;
      ...
      <div class="listado-card" ${accion}>
        <div class="listado-card-top">
          ...
          <div class="listado-info">
            <div class="listado-nombre">
              ${item.nombre} ${badgeHTML}
            </div>
            <div class="listado-tipo">🍴 ${item.tipo}</div>
            ${esFeria && item._dist ? `<div class="listado-distancia">📍 ${item._dist} ${t.distancia}</div>` : ''}
            ${esFeria ? `<div class="listado-dias">📅 ${item.dias[currentLang]} · ⏰ ${item.horario}</div>` : ''}
            ${!esFeria ? `<div class="listado-distancia">${distStr}</div>` : ''}
          </div>
...
function getMP3Path(circuitIdx, paradaIdx) {
  return `assets/audio/c${circuitIdx + 1}-p${paradaIdx + 1}.mp3`;
}
...
function toggleAudio() {
  ...
  else if (isPlaying && !isPaused) pauseAudio();
...
function skipForward() {
  ...
  if (usandoMP3 && audioElement) {
    ...
    if (currentParada < totalParadas - 1) { speechSynth.cancel(); if (isPlaying) playParada(currentParada + 1); }
  }
  updatePlayerControls();
}
...
function skipBackward() {
  ...
  if (usandoMP3 && audioElement) {
    ...
    if (currentParada > 0) { speechSynth.cancel(); if (isPlaying) playParada(currentParada - 1); }
  }
  updatePlayerControls();
}
...
function loadVoices() {
  ...
  const langCodes = { es: ['es-AR','es-MX','es-US','es-ES','es'], en: ['en-US','en-GB','en'], pt: ['pt-BR','pt-PT','pt'] };
  const maleKW    = ['jorge','diego','carlos','miguel','pablo','male','david','daniel','james','google español','google us english male'];
  const codes     = langCodes[currentLang] || ['es'];
  ...
  for (const code of codes) {
    const v = voices.find(v => v.lang.toLowerCase().startsWith(code.toLowerCase()) && v.name.toLowerCase().includes('google'));
    if (v) { selectedVoice = v; return; }
  }
  for (const code of codes) {
...
function hablarFragmento(fIdx, paradaIdx) {
  if (!isPlaying || fIdx >= fragmentos.length) {
    if (isPlaying) playTTSParada(paradaIdx + 1); return;
  }
  fragIdx = fIdx;
  loadVoices();
  const utt   = new SpeechSynthesisUtterance(fragmentos[fIdx]);
